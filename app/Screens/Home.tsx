...
export default function Home({ navigation }: HomeProps) {
    ...
    const emitLocationUpdate = () => {
      if (socketRef.current?.connected && location && isActiveTrip) {
        const locationData = {
          ...
          // Note: "accuracy", "altitude", "timestamp" are not in the spec
        };


        socketRef.current.emit("driver-location-update", locationData);

        console.log('all location', locationData)
        ...
        console.log(`🧭 Heading: ${location.coords.heading || 0}°`);
        console.log(`🚀 Speed: ${location.coords.speed || 0} m/s`);
  ...
  }, [location, isActiveTrip, driverID]);
  ...
  useEffect(() => {
    const fetchDrivers = async () => {
      try {
        ...
        const matchingDriver = data.drivers.find((driver: any) => driver.driverID === driverID);

        if (!matchingDriver) {
          console.warn("No matching driver found with ID:", driverID);
          setBusRoute([]);
          return;
        }
  ...
  const switchStatus = async () => {
    ...
    try {
      ...
      console.log(`Driver ${driverID} is now ${!isActiveTrip ? "active" : "inactive"}`);
  ...
  useEffect(() => {
    ...
    const initializeSocket = async () => {
      ...
      console.log("Attempting to connect to socket with driverID:", driverID);
  ...
  }, [driverID, busID, startPoint, endPoint]);
